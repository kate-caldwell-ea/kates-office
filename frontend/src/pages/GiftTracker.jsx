...
export default function GiftTracker() {
  ...
  // Group by recipient and sort by upcoming birthday
  const byRecipient = {}
  gifts.forEach(g => {
  ...
  const sortedRecipients = Object.keys(byRecipient).sort((a, b) => {
    const aGift = byRecipient[a][0]
    const bGift = byRecipient[b][0]
    const aDays = getDaysUntilBirthday(aGift.birthday) ?? 999
    const bDays = getDaysUntilBirthday(bGift.birthday) ?? 999
    return aDays - bDays
  })
  ...
  // Stats
  const totalBudget = gifts.reduce((sum, g) => sum + (g.cost || g.budget_max || 0), 0)
  const pendingGifts = gifts.filter(g => !['delivered', 'shipped'].includes(g.status)).length
  const nextBirthday = sortedRecipients.length > 0 ? byRecipient[sortedRecipients[0]][0] : null
  const nextDays = nextBirthday ? getDaysUntilBirthday(nextBirthday.birthday) : null
    ...
    <div className="space-y-6">
      ...
      <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-text-400 mt-1">Birthday pipeline, budgets, and status tracking — {new Date().getFullYear()}</p>
          </div>
      ...
      <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="card">
          ...
          <p className="text-2xl font-semibold text-gold-400 mt-1">{pendingGifts}</p>
        </div>
        <div className="card">
          <p className="text-xs text-text-500">Total Spent/Budget</p>
          <p className="text-2xl font-semibold text-text-200 mt-1">${totalBudget.toFixed(0)}</p>
        </div>
        <div className="card">
          ...
          {nextBirthday && (
            <p className="text-xs text-text-500 mt-0.5">{nextBirthday.recipient} — {formatBirthday(nextBirthday.birthday)}</p>
          )}
        </div>
      </motion.div>
      ...
      <motion.div custom={2} variants={fadeUp} initial="hidden" animate="visible" className="flex gap-2 flex-wrap">
        ...
        {statusPipeline.map(s => {
          const count = gifts.filter(g => g.status === s.value).length
      ...
      {gifts.length === 0 ? (
        <motion.div custom={3} variants={fadeUp} initial="hidden" animate="visible" className="card text-center py-12">
          <Gift className="w-12 h-12 text-text-600 mx-auto mb-3" />
          <h3 className="text-text-300 text-lg font-medium">No gifts tracked yet</h3>
          <p className="text-text-500 text-sm mt-1">Add family members' birthdays and start tracking gift ideas, purchases, and budgets.</p>
        </motion.div>
      ) : filter === 'all' ? (
        // Grouped by recipient
        sortedRecipients.map((recipient, ri) => {
          const recipientGifts = byRecipient[recipient]
          const bday = recipientGifts[0].birthday
          const daysUntil = getDaysUntilBirthday(bday)

          return (
            <motion.div key={recipient} custom={3 + ri} variants={fadeUp} initial="hidden" animate="visible" className="card">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {daysUntil !== null && daysUntil <= 30 && (
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      daysUntil <= 7 ? 'bg-red-500/15 text-red-400' : 'bg-gold-500/15 text-gold-400'
                    }`}>
              ...
              <div className="space-y-2">
                {recipientGifts.map(gift => (
                  <GiftRow key={gift.id} gift={gift} onEdit={() => { setEditingGift(gift); setShowModal(true); }} onDelete={() => handleDelete(gift.id)} onStatusUpdate={handleStatusUpdate} />
                ))}
              </div>
            </motion.div>
          )
        ...
        <div className="space-y-2">
          {filteredGifts.map((gift, i) => (
            <motion.div key={gift.id} custom={3 + i} variants={fadeUp} initial="hidden" animate="visible" className="card">
              <GiftRow gift={gift} onEdit={() => { setEditingGift(gift); setShowModal(true); }} onDelete={() => handleDelete(gift.id)} onStatusUpdate={handleStatusUpdate} />
            </motion.div>
          ))}
          {filteredGifts.length === 0 && (
            <div className="card text-center py-8">
              <p className="text-text-500">No gifts with this status</p>
            </div>
          )}
        </div>
      )}
    ...
    <div className="flex items-center gap-3 p-3 rounded-xl bg-dark-600/50 hover:bg-dark-500/50 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-medium text-text-200 truncate">{gift.gift_idea || 'TBD'}</p>
        ...
        <div className="flex items-center gap-3 text-xs text-text-500 mt-0.5">
          {gift.cost ? <span>${gift.cost.toFixed(0)}</span> : gift.budget_max ? <span>Budget: ${gift.budget_min || 0}-${gift.budget_max}</span> : null}
          {gift.purchase_url && (
          ...
          {gift.notes && <span className="truncate">{gift.notes}</span>}
        </div>
      </div>
      ...
      {gift.status !== 'delivered' && (
        <button
          ...
          className="hidden sm:block px-3 py-1.5 rounded-lg bg-dark-500/50 text-text-400 text-xs hover:bg-dark-400/50 transition-colors"
